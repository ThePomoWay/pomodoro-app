...
import { createProjectEndpoint, createSectionEndpoint, createTaskEndpoint, deleteProjectEndpoint, getAllProjectEndpoint, rearrangeTasksInProjectEndpoint, updateProjectEndpoint } from "./Endpoints";
import { NetworkService } from "./NetworkService";

export function getAllProjectsApi() {
    let endpoint = getAllProjectEndpoint.replace('{userId}', AuthService.getUserId());
...
export function createSectionApi(project, section) {
    ...
    return NetworkService.post(endpoint, {}, {title: section.title, to: [], fid: section.fid});
}

export function rearrangeTaskApi(obj) {
    let endpoint = rearrangeTasksInProjectEndpoint.replace('{userId}', AuthService.getUserId())
                                                  .replace('{projectId}', obj.projectId)
                                                  .replace('{taskId}', obj.taskId);
    return NetworkService.patch(endpoint, {}, {source: obj.source, destination: obj.destination, isSame: obj.isSame});
}