...
export async function syncIdb() {
    ...
    console.log('Tasks from IDB', localTasks);

    let inboxId = AuthService.getProjectId();
    ...
    for(let task of localTasks) {
        if(!task._id) {
            ...
            if(response && response.data && response.data.tid) {
                store.dispatch(updateLocalTaskThunk({...task, _id: response.data.tid}));
            }
            else {
    ...
    let beTodaysTasks = todaysTasks.map(item => tasksObj[item] && tasksObj[item]._id).filter(i=>i);
        ...
        if(res && res.success){
            ...
            if(res && res.success){
                ...
                for(let project of response.data.projects) {
                    store.dispatch(createLocalProjectAsync({project: processBEProject(project, tasksObj)}));
                }
            }
        }
    }
