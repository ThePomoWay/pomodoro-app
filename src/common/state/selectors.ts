...
export const selectCompletedTaskInProject = (projectId, sectionId) => {
    return (state) => {
        return Object.keys(state.tasks.tasks).filter(item => state.tasks.tasks[item].isComplete && state.tasks.tasks[item].project.projectID === projectId && state.tasks.tasks[item].project.secID === sectionId).map(item => state.tasks.tasks[item]);
    }
}
...
export const selectDefaultTimes = (state) => {return {defaultWorkTime: state.timer.defaultWorkTime,
