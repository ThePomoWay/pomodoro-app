...
let filter = function(filterArr, filterVal) {
    ...
    return filterArr.filter(item => item.title.toLowerCase().includes(filterVal.toLowerCase()));
}
    ...
    let getTags = useCallback(() => {
        if(filteredTags.length > 0) {
            return (<div className={`${styles['tags-list']} `}>
                        { tags.map((tag, ind) => (<div key={ind} onClick={(e) => handleCheckboxClick(tag)} className={`popover-normal-item ${styles["tags-item"]} ${props.hideSelect && styles['view-only']} ${selectedTagsObj[tag.fid] && "popover-normal-item-selected"}`}>
                            
                            <svg width="12" height="12" viewBox="0 0 12 12" style={{fill: tag.color}} xmlns="http://www.w3.org/2000/svg">
                            ...
                            <span style={{color: tag.color}} className="">{tag.title} </span>
                            {!!selectedTagsObj[tag.fid] && (<Done style={{width: '0.6em', "fill": tag.color, position: 'absolute', right: '8px'}}></Done>)}
                            {/* {
                                (!props.hideSelect && (
        ...
        return (<span className='popover-unselectable'>{(tagTitle.length > 0 && tags && tags.length !== 0) && "No tags found" || "Your tags will appear here"}</span>);
    });
            ...
            <div className={styles['create-tags-container']}>
                {
                    ...
                    (<span className={`text-small text-gray ${styles["create-text"]}`} onClick={(e) => setShowCreateTag(!showCreateTag)}>
                        <Add></Add>
                        Create new label
                    </span>)
...
}
{
    !showCreateTag && (
        <div>
            <input className={styles['tag-input']} autoFocus onChange={onInput} onKeyDown={onKeyDown} value={tagTitle} />
            {!createLabelView && (<Add className={styles['tag-svg']} onClick={(e) => setCreateLabelView(true)}></Add>)}
        </div>
    )
}
{                   createLabelView &&
                    ...
                    (
                        <div className={styles['create-view']}>
                            <p className={styles["tag-name-label"]}>
                                Tag Name:
                            </p>
                            ...
                            <div className={styles['color-palette']}>
                                {tagColorPalette.map((item, index) => (<span onClick={(e) => setSelectedIndex(index)} style={{"backgroundColor": item}} className="circle-simple flex flex-center" key={index}>{selectedIndex === index && (<Done style={{width: '0.6em', "fill": "white"}}></Done>)}</span>))}
                            </div>
