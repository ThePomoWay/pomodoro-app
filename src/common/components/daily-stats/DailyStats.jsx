        ...
        <div className={styles["completed-pomos"]}>
            ...
            {ePomos === 0 && cPomos === 0 && 
                ...
                <div className={styles["circles"]}>
                    {[...Array(cPomos)].map((item, index) => (<div key={`completed-pomo-${index}`} className={styles["completed-pomo"]+' '+styles["filled"]}>{index+1}</div>))}
                    {ePomos > cPomos && [...Array(ePomos - cPomos)].map((item, index) => (<div key={`pending-pomo-${index}`} className={styles["estimated-pomo"]}></div>)) }
                </div>
            }
